...
const DEFAULT_SETTINGS: TauriSettings = INJECT_FALLBACK_SETTINGS;
...
async function checkAndReportMeetings(meta: {
  ...
  if (overlay) {
    ...
    const nextMeeting = getNextJoinableMeeting(result.meetings, {
      gracePeriodMinutes:
        settings?.maxMinutesAfterStart ??
        INJECT_FALLBACK_SETTINGS.maxMinutesAfterStart,
      alreadyJoined: joinedMeetings,
      suppressedMeetings,
      joinBeforeMinutes:
        settings?.joinBeforeMinutes ?? INJECT_FALLBACK_SETTINGS.joinBeforeMinutes,
    });
  ...
  if (lastMeetings.length > 0) {
    void syncJoinedMeetings().then(() => {
      const nextMeeting = getNextJoinableMeeting(lastMeetings, {
        ...
        suppressedMeetings,
        joinBeforeMinutes:
          settings?.joinBeforeMinutes ??
          INJECT_FALLBACK_SETTINGS.joinBeforeMinutes,
      });
      overlay?.update(nextMeeting);
    });
  }
...
function startFallbackInterval(): void {
  ...
  const intervalSeconds = Math.max(
    settings?.checkIntervalSeconds ||
      INJECT_FALLBACK_SETTINGS.checkIntervalSeconds,
    1
  );
...
function startJoinCountdown(): void {
  ...
  const seconds =
    settings.joinCountdownSeconds ?? INJECT_FALLBACK_SETTINGS.joinCountdownSeconds;
