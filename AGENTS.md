...
- `scripts`: Workspace helpers like `bootstrap` and app launch wrappers.

## Build, Test, and Development Commands
- `pnpm run bootstrap`: Install dependencies and prep the workspace (including Rust crates).
- `pnpm run dev`: Launch the Tauri app in dev mode.
...
- `pnpm run test:extension:e2e`: Run Playwright E2E tests for the extension.
...
- Rust tests live under `packages/tauri/src-tauri` and run via `cargo test` (see `pnpm run test:tauri`).
